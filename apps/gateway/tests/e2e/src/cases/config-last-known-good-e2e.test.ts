...
describe("config last-known-good: rejected updates keep serving across resync and restart", () => {
  ...
  const etcdPrefix = `/sibyl-gateway-e2e-lkg-${randomUUID()}`;
  let modelId: string;
  let modelKey: string;
  ...
  beforeAll(async () => {
    ...
    cacheDir = await mkdtemp(join(tmpdir(), "sibyl-gateway-lkg-cache-"));
    app = await spawnApp({
      etcdPrefix,
  ...
  test("last-good lifecycle: reject, restore, disable persistence, and delete", async (ctx) => {
...

    {
      ...
      expect(text).toMatch(/sibyl_gateway_config_stale_served_resources\{kind="models"\} 1/);
    }
...

    {
      ...
      const text = await scrape(app);
      expect(text).toMatch(/sibyl_gateway_config_stale_served_resources\{kind="models"\} 0/);
    }
  });
});
