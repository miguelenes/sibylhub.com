...
//
//   sibyl_gateway_llm_{input,output,total}_tokens_total  chat + messages
//   sibyl_gateway_llm_spend_micro_usd_total              chat + messages
//   sibyl_gateway_llm_tokens_by_client_total             chat + messages + responses
//   sibyl_gateway_tokens_consumed_total (legacy)         chat ALONE
//
...
describe("token metrics endpoint coverage e2e", () => {
  ...
  test("/v1/responses reports its tokens on every token family", async (ctx) => {
    ...
    expect(delta(before, text, "sibyl_gateway_llm_input_tokens_total", "/v1/responses")).toBe(11);
    expect(delta(before, text, "sibyl_gateway_llm_output_tokens_total", "/v1/responses")).toBe(13);
    expect(delta(before, text, "sibyl_gateway_llm_total_tokens_total", "/v1/responses")).toBe(24);
    ...
    expect(
      text
        ...
        .filter(
          (l) =>
            l.startsWith("sibyl_gateway_tokens_consumed_total{") &&
  ...
  test("/v1/embeddings reports its tokens too", async (ctx) => {
    ...
    const input = seriesFor(text, "sibyl_gateway_llm_input_tokens_total", "/v1/embeddings");
    ...
    expect(
      seriesFor(text, "sibyl_gateway_llm_output_tokens_total", "/v1/embeddings"),
    ).toHaveLength(0);
    ...
    expect(
      text
        ...
        .filter(
          (l) =>
            l.startsWith("sibyl_gateway_llm_tokens_by_client_total{") &&
  ...
  test("token labels line up with the request families", async (ctx) => {
    ...
    const req = seriesFor(text, "sibyl_gateway_llm_requests_total", "/v1/responses")[0];
    const tok = seriesFor(text, "sibyl_gateway_llm_input_tokens_total", "/v1/responses")[0];
