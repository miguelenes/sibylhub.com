...
const DD_SERVICE = "sibyl-gateway-e2e";
...
describe("datadog exporter e2e (#688): DP delivers a gzip JSON intake to Datadog", () => {
  ...
  test(
    ...
    async (ctx) => {
      ...
      // A model dimension is present under a GenAI / sibyl-gateway key (the upstream
      ...
      const logTraceId = log["sibyl-gateway.trace_id"];
      expect(logTraceId).toMatch(/^[0-9a-f]{32}$/);
      const requestId = log["sibyl-gateway.request_id"];
      const deadline = Date.now() + 10_000;
      while (
        Date.now() < deadline &&
        !otlp.spans.some((s) => s.attributes["sibyl-gateway.request_id"] === requestId)
      ) {
        await new Promise((r) => setTimeout(r, 50));
      }
