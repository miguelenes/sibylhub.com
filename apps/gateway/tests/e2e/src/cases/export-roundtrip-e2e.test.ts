...
// E2E: the `sibyl-gateway export` round-trip — the migration path that closes the
...
// `sibyl-gateway export` reads that etcd store and emits a resources.yaml; a
...
describe("sibyl-gateway export: etcd → export → file round-trip", () => {
  ...
  test("exported file has no live secret and reproduces the gateway's behavior", async (ctx) => {
    ...
    // 1) Run `sibyl-gateway export` against the seeded etcd store.
  ...
  test("default export redacts a live secret on every secret-bearing kind", async (ctx) => {
    ...
    const etcd = new EtcdClient();
    const prefix = `/sibyl-gateway-export-noleak-${randomUUID()}`;
    ...
    try {
      ...
      expect(yaml).toContain("${SIBYLSECRET");
