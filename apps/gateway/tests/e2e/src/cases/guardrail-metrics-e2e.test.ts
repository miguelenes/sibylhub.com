...
// `sibyl_gateway_guardrail_latency_seconds` (AISIX-Cloud#1076). Every guardrail
...
/**
 * Sum the `sibyl_gateway_guardrail_latency_seconds_count` values of every series
...
describe("guardrail latency metrics e2e: per-execution histogram", () => {
  ...
  test("clean request records result=allowed for every consulted guardrail, as a real bucketed histogram", async (ctx) => {
    ...
    expect(after).toMatch(
      /sibyl_gateway_guardrail_latency_seconds_count\{[^}]*guardrail="metrics-moderation"[^}]*kind="openai_moderation"/,
    );
    expect(after).toMatch(
      /sibyl_gateway_guardrail_latency_seconds_count\{[^}]*guardrail="metrics-kw-input"[^}]*kind="keyword"/,
    );
    expect(after).toMatch(
      /sibyl_gateway_guardrail_latency_seconds_bucket\{[^}]*le="0.001"/,
    );
    expect(after).toMatch(/sibyl_gateway_guardrail_latency_seconds_bucket\{[^}]*env_id="/);
  });
  ...
  test("aggregate block counter covers Chat, Messages, and Responses", async (ctx) => {
    ...
    let previous = counterValue(body, "sibyl_gateway_guardrail_blocks_total");
    for (const protocol of ["chat", "messages", "responses"] as const) {
      ...
      body = await scrape();
      const current = counterValue(body, "sibyl_gateway_guardrail_blocks_total");
  ...
  test("aggregate bypass counter covers Chat, Messages, and Responses", async (ctx) => {
    ...
    let body = await scrape();
    let previous = counterValue(body, "sibyl_gateway_guardrail_bypasses_total", labels);
    for (const protocol of ["chat", "messages", "responses"] as const) {
      ...
      body = await scrape();
      const current = counterValue(body, "sibyl_gateway_guardrail_bypasses_total", labels);
