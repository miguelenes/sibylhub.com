...
describe("audio duration cost basis (#1138)", () => {
  ...
  test("a duration-billed transcription reports its audio length", async (ctx) => {
    ...
    const reportedSpan = await waitSpan(
      otlp,
      reported.headers.get("x-sibylhub-request-id") ?? "",
    );
    expect(
      Number(reportedSpan["sibyl-gateway.audio.duration_seconds"]),
    ...
    const plainSpan = await waitSpan(
      otlp,
      plain.headers.get("x-sibylhub-request-id") ?? "",
    );
    expect(
      Number(plainSpan["sibyl-gateway.audio.duration_seconds"]),
