...
describe("model group member rate limit e2e (AISIX-Cloud#1087)", () => {
  ...
  test("over-limit member fails over to the next target (RPM)", async (ctx) => {
    ...
    expect(delta("sibyl_gateway_deployment_requests_total", { model: "mgrl-limited" })).toBe(0);
    expect(
      delta("sibyl_gateway_deployment_failure_responses_total", { model: "mgrl-limited" }),
    ).toBe(0);
    expect(delta("sibyl_gateway_deployment_requests_total", { model: "mgrl-backup" })).toBe(1);
    expect(
      delta("sibyl_gateway_deployment_success_responses_total", { model: "mgrl-backup" }),
    ).toBe(1);
    expect(
