...
// `crates/sibyl-gateway-core/src/models/cache_policy.rs` and accepted by
...
//     `x-sibylhub-cache: hit` at t=1 and `x-sibylhub-cache: miss` at
...
// `docs/api-proxy.md` §3 (`x-sibylhub-cache` response header).
...
describe("cache TTL eviction e2e: entry expires after ttl_seconds", () => {
  ...
  test(
    ...
    async (ctx) => {
      ...
      // a CachePolicy in scope the gateway emits `x-sibylhub-cache:
      ...
      expect(third.headers.get("x-sibylhub-cache")).toBe("miss");
      await third.text();
