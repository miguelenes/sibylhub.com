...
describe("/v1/chat/completions anthropic streaming input tokens (#450)", () => {
  ...
  test("records non-zero input_tokens on streaming chat completions (#450)", async (ctx) => {
    ...
    while (Date.now() < deadline) {
      ...
      inTok = sumMetric(scrape, "sibyl_gateway_llm_input_tokens_total", "/v1/chat/completions");
      outTok = sumMetric(scrape, "sibyl_gateway_llm_output_tokens_total", "/v1/chat/completions");
