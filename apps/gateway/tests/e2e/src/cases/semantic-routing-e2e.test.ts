...
// when none clears its threshold. No CP involved: real `sibyl-gateway` binary +
...
describe("semantic routing e2e", () => {
  ...
  test("routes a matching prompt to its route target and sets x-sibylhub-route", async (ctx) => {
  ...
  test("falls through to default when no route clears its threshold (no x-sibylhub-route)", async (ctx) => {
    if (!etcdReachable || !app || !seed) {
      ctx.skip();
      return;
    }
