...
describe("usage estimation e2e (AISIX-Cloud#1074): missing upstream usage is locally counted and flagged", () => {
  ...
  async function waitTokenMetrics(model: string): Promise<{ input: number; output: number }> {
    ...
    while (Date.now() < deadline) {
      input = await tokenMetric("sibyl_gateway_llm_input_tokens_total", model);
      output = await tokenMetric("sibyl_gateway_llm_output_tokens_total", model);
      if (input > 0) break;
  ...
  test("chat SSE without a usage chunk: tokens estimated, flagged, client stream untouched", async (ctx) => {
    ...
    const inputBefore = await tokenMetric("sibyl_gateway_llm_input_tokens_total", "est-chat-stream");
    ...
    let output = 0;
    while (Date.now() < deadline) {
      input = await tokenMetric("sibyl_gateway_llm_input_tokens_total", "est-chat-stream");
      output = await tokenMetric("sibyl_gateway_llm_output_tokens_total", "est-chat-stream");
  ...
  test("chat non-streaming 200 without usage: the client usage block carries the same estimate the record does", async (ctx) => {
    ...
    const inputBefore = await tokenMetric("sibyl_gateway_llm_input_tokens_total", "est-chat-nonstream");
    ...
    let output = 0;
    while (Date.now() < deadline) {
      input = await tokenMetric("sibyl_gateway_llm_input_tokens_total", "est-chat-nonstream");
      output = await tokenMetric("sibyl_gateway_llm_output_tokens_total", "est-chat-nonstream");
      if (input > inputBefore) break;
      await new Promise((r) => setTimeout(r, 100));
    }
    expect(input - inputBefore).toBe(EXPECTED_PROMPT_TOKENS);
  ...
  test("/v1/messages SSE with no usage anywhere: estimated and flagged", async (ctx) => {
    ...
    const inputBefore = await tokenMetric("sibyl_gateway_llm_input_tokens_total", "est-msgs-stream");
    const outputBefore = await tokenMetric("sibyl_gateway_llm_output_tokens_total", "est-msgs-stream");
    ...
    let output = 0;
    while (Date.now() < deadline) {
      input = await tokenMetric("sibyl_gateway_llm_input_tokens_total", "est-msgs-stream");
      output = await tokenMetric("sibyl_gateway_llm_output_tokens_total", "est-msgs-stream");
      if (input > inputBefore) break;
      await new Promise((r) => setTimeout(r, 100));
    }
