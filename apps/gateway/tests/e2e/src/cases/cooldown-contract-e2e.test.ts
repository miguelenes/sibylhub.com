...
describe("cooldown observability — a cooldown transition emits sibyl_gateway_deployment_* metrics", () => {
  let app: SpawnedApp | undefined;
  let admin: AdminClient | undefined;
  let seed: SeedClient | undefined;
  let etcdReachable = false;
  ...
  test("the deployment cooldown counter and state gauge appear on the scrape with the cooled target's labels", async (ctx) => {
    ...
    const cooledLine = text
      ...
      .find(
        (l) =>
          l.startsWith("sibyl_gateway_deployment_cooled_down_total{") &&
    ...
    const stateLine = text
      ...
      .find(
        (l) =>
          l.startsWith("sibyl_gateway_deployment_state{") &&
          l.includes('model="cd-metrics-500-model"'),
      );
...
/** Value of the `sibyl_gateway_deployment_state` series carrying `model="<model>"`. */
