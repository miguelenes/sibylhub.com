...
//
// Per `docs/api-proxy.md` §3 (PR #191) the `x-sibylhub-cache` response
...
//      `x-sibylhub-cache` header — bypass is silent, not a `miss` or
//      `disabled` label.
//
...
describe("cache streaming bypass e2e: streaming responses are never cached", () => {
  ...
  test(
    "streaming: no x-sibylhub-cache header, upstream re-hit on every call",
    async (ctx) => {
      ...
      // (waits for x-sibylhub-cache: miss like other cache-edge tests).
      ...
      // x-sibylhub-cache (header-absent per docs §3).
      ...
      expect(s1.headers.get("x-sibylhub-cache")).toBeNull();
      ...
      expect(s2.headers.get("x-sibylhub-cache")).toBeNull();
      await s2.text();
      ...
      expect(n1.headers.get("x-sibylhub-cache")).toBe("miss");
      await n1.text();
      ...
      expect(n2.headers.get("x-sibylhub-cache")).toBe("hit");
      await n2.text();
