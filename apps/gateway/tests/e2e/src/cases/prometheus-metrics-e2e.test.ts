...
describe("prometheus metrics e2e", () => {
  ...
  test("scrape contains SibylHub Gateway-native request and token metrics", async (ctx) => {
    ...
    expect(text).toContain("sibyl_gateway_proxy_requests_total");
    expect(text).toContain("sibyl_gateway_llm_requests_total");
    expect(text).toContain("sibyl_gateway_llm_input_tokens_total");
    expect(text).toContain("sibyl_gateway_llm_output_tokens_total");
    expect(text).toContain("sibyl_gateway_llm_total_tokens_total");
    expect(text).toContain("sibyl_gateway_proxy_in_flight_requests");
    expect(text).toMatch(
      /sibyl_gateway_proxy_requests_total\{[^}]*endpoint="\/v1\/chat\/completions"[^}]*model="prometheus-gpt"[^}]*status="200"/,
    );
    expect(text).toMatch(
      /sibyl_gateway_llm_requests_total\{[^}]*endpoint="\/v1\/chat\/completions"[^}]*model="prometheus-gpt"[^}]*status="200"/,
    );
  ...
  test("custom prometheus path is used for scrapes", async (ctx) => {
    ...
    try {
      ...
      const text = await scrape.text();
      expect(text).toMatch(
        /sibyl_gateway_proxy_requests_total\{[^}]*endpoint="\/v1\/chat\/completions"[^}]*model="prometheus-custom-gpt"/,
      );
      expect(text).toContain("sibyl_gateway_llm_total_tokens_total");
    } finally {
  ...
  // `sibyl_gateway_usage_events_emitted_total{handler="chat", status_code="2xx",
  ...
  test("usage_events_emitted counter increments on successful chat (#408)", async (ctx) => {
    ...
    expect(after).toContain("sibyl_gateway_usage_events_emitted_total");
    ...
    expect(after).toMatch(
      /sibyl_gateway_usage_events_emitted_total\{[^}]*handler="chat"[^}]*\}/,
    );
    expect(after).toMatch(
      /sibyl_gateway_usage_events_emitted_total\{[^}]*status_code="2xx"[^}]*\}/,
    );
    expect(after).toMatch(
      /sibyl_gateway_usage_events_emitted_total\{[^}]*inbound_protocol="openai"[^}]*\}/,
    );
    ...
    expect(after).not.toMatch(
      /sibyl_gateway_usage_events_emitted_total\{[^}]*status_code="200"/,
    );
    ...
    expect(after).toMatch(
      /sibyl_gateway_usage_events_emitted_total\{[^}]*[,{]status="200"/,
    );
  ...
  // SibylHub Gateway-native series after real proxy traffic, no admin routes leak
  ...
  test("metrics listener is the only scrape surface", async (ctx) => {
    ...
    // Drive traffic so the SibylHub Gateway-native series exist regardless of
...
/**
 * Extract the integer value of one `sibyl_gateway_usage_events_emitted_total`
