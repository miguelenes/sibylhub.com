...
// end against a real `sibyl-gateway` binary:
//
//   1. A file uploaded with a routing hint (x-sibylhub-model header — the
//      OpenAI SDK path; multipart `model` field is the raw-HTTP path)
//      returns a GATEWAY id (`sibyl-gateway-…`) embedding the Model.
...
describe("jobs e2e: /v1/files + /v1/batches + /v1/fine_tuning/jobs (#720)", () => {
    ...
    const hdrPk = await seed.createProviderKey({
      ...
      request: {
        default_headers: {
          ...
          "x-sibylhub-request-id": "operator-should-not-win",
        },
      },
    });
    await seed.createModel({
  ...
  test("full batch lifecycle through the OpenAI SDK: upload → create → retrieve → content → cancel", async (ctx) => {
    ...
    const file = await client.files.create(
      ...
      { headers: { "x-sibylhub-model": "jobs-e2e-model" } },
    );
    expect(file.id.startsWith("sibyl-gateway-")).toBe(true);
    ...
    expect(batch.id.startsWith("sibyl-gateway-")).toBe(true);
    ...
    expect(got.output_file_id?.startsWith("sibyl-gateway-")).toBe(true);
  ...
  test("fine-tuning: encoded training_file routes the job, provider base model forwards verbatim", async (ctx) => {
    ...
    expect(job.id.startsWith("sibyl-gateway-")).toBe(true);
  ...
  test("raw multipart `model` form field routes the upload (non-SDK path)", async (ctx) => {
    ...
    expect(body.id.startsWith("sibyl-gateway-")).toBe(true);
  });
  ...
  test("provider-key headers reach the jobs upstream without doubling gateway headers", async (ctx) => {
    ...
    const rid = headers["x-sibylhub-request-id"] ?? "";
