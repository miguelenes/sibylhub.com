    ...
    'targets: for (target_idx, target) in attempt_models.iter().enumerate() {
        ...
        for attempt_idx in 0..=budget.attempts {
            ...
            let result = if crate::dispatch::serves_natively(
                snapshot,
                &target.model,
                sibyl_gateway_core::ApiSurface::Responses,
            ) {
                responses_to_target(
                    state,
                    snapshot,
...
async fn responses_to_target(
    ...
    if is_stream {
                    ...
                    Some(d) => tokio::time::timeout(d, stream.next())
                        ...
                        .map_err(|_| {
                            crate::cooldown::note_failure(
                                ...
                                model_id,
                                model.cooldown.as_ref(),
                                sibyl_gateway_hub::BridgeError::Timeout {
        ...
        let eos_scan = sibyl_gateway_guardrails::Guardrail::runs_on_output(chain)
...
/// [`Bridge`](sibyl_gateway_hub::Bridge), then re-encoding the response back
...
pub(crate) fn responses_in_band_error(event: &Value) -> Option<sibyl_gateway_hub::BridgeError> {
    ...
    Some(
        ...
        .unwrap_or_else(|| sibyl_gateway_hub::BridgeError::UpstreamInBand {
            status: None,
...
impl<F: FnOnce(ResponseUsage, String, Vec<sibyl_gateway_core::GuardrailMonitorHit>)>
...
impl<F: FnOnce(ResponseUsage, String, Vec<sibyl_gateway_core::GuardrailMonitorHit>)> Drop
...
where
    ...
    F: FnOnce(ResponseUsage, String, Vec<sibyl_gateway_core::GuardrailMonitorHit>) + Send + 'static,
{
...
/// `x-sibylhub-request-id` header. Shared by the streaming verbatim-passthrough
...
fn emit_usage_event(
    ...
    // `sibyl_gateway_llm_*_tokens_total` families on the same path.
...
mod tests {
    ...
    fn responses_input_maps_tool_loop_items_to_assistant_and_tool() {
        ...
        assert_eq!(seen[0].0, sibyl_gateway_hub::Role::System);
        assert_eq!(seen[1].0, sibyl_gateway_hub::Role::User);
    ...
    fn responses_input_keeps_a_text_empty_assistant_item_as_a_boundary() {
        ...
        assert_eq!(
            roles,
            vec![
                sibyl_gateway_hub::Role::User,
                sibyl_gateway_hub::Role::Assistant,
                sibyl_gateway_hub::Role::User,
            ],
        );
        assert_eq!(chat.messages[1].content_str(), "", "no readable text");
        let window = sibyl_gateway_guardrails::latest_turn_view(&chat);
    ...
    fn a_replayed_tool_result_is_not_user_role_on_either_surface() {
        ...
        let chat: sibyl_gateway_hub::ChatFormat = serde_json::from_value(serde_json::json!({
            "model": "m",
            "messages": [
        ...
        .unwrap();
        assert_eq!(responses.messages[0].role, sibyl_gateway_hub::Role::Tool);
        assert_eq!(
    ...
    fn responses_input_maps_a_tool_result_to_the_tool_role() {
        ...
        assert_eq!(chat.messages[1].role, sibyl_gateway_hub::Role::Tool);
    ...
    fn openai_pk_with_overrides(api_base: &str) -> ResourceEntry<sibyl_gateway_core::ProviderKey> {
        let json = format!(
    ...
    fn openai_pk_tagged(api_base: &str) -> ResourceEntry<sibyl_gateway_core::ProviderKey> {
        let json = format!(
    ...
    fn new_snap_openai_tagged(api_base: &str) -> GatewaySnapshot {
        let snap = GatewaySnapshot::new();
        snap.provider_keys.insert(openai_pk_tagged(api_base));
        snap
    }

    fn anthropic_pk_at(api_base: &str) -> ResourceEntry<sibyl_gateway_core::ProviderKey> {
        let json = format!(
    ...
    fn new_snap_anthropic_at(api_base: &str) -> GatewaySnapshot {
        let snap = GatewaySnapshot::new();
    ...
    async fn monitor_scan_disconnect_still_emits_usage_event() {
        ...
        let g: sibyl_gateway_core::Guardrail = serde_json::from_str(&textmod_json).unwrap();
...
data: [DONE]\n\n";
    ...
    fn masking_guardrail() -> ResourceEntry<sibyl_gateway_core::models::Guardrail> {
    ...
    #[track_caller]
    fn assert_masked_by_eda(event: &sibyl_gateway_obs::UsageEvent, spans: u32) {
