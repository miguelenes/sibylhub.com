...
use sibyl_gateway_core::models::BackgroundModelCheck;
use sibyl_gateway_core::{GatewaySnapshot, Model};
use sibyl_gateway_hub::{BridgeError, ChatFormat, ChatMessage, Hub};
...
fn upstream_template_is_probeable(model: &sibyl_gateway_core::Model) -> bool {
    model
        .model_name
        .as_deref()
...
mod tests {
    ...
    fn upstream_template_probeability() {
        let model = |model_name: Option<&str>| -> sibyl_gateway_core::Model {
    ...
    async fn background_check_marks_unhealthy_on_failure() {
        ...
        let snapshot = Arc::new(GatewaySnapshot::new());
        snapshot
            .provider_keys
