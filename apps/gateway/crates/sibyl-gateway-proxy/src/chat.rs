        ...
        Ok(mut success) => {
            ...
            if req.is_streaming() && success.telemetry_handled_by_stream {
                ...
                emit_access_log(
                    ...
                    &success.routing,
                    None,
                    Some(sibyl_gateway_obs::CacheAccessLog {
                    ...
                    Err(err) => {
                        tracing::warn!(
                            ...
                            "target display_name is not a valid HTTP header value; \
                             omitting x-sibylhub-served-by — rename the target to use \
                             only visible ASCII (no CR/LF, no non-ASCII characters)"
                        );
                    }
                }
            }
            // `x-sibylhub-route` exposes which semantic route matched (#641).
            ...
            if let Some(route) = success.served_by_route.as_deref() {
                match axum::http::HeaderValue::try_from(route) {
                    Ok(v) => {
                        success.response.headers_mut().insert("x-sibylhub-route", v);
                    }
                    Err(err) => {
                        tracing::warn!(
                            route_name = %route,
                            ...
                            "semantic route name is not a valid HTTP header value; \
                             omitting x-sibylhub-route — rename the route to use only \
...
struct Success {
    ...
    /// the dashboard's /logs column. See `sibyl-gateway-core::CachePolicy`
    /// (Stage 2) and `sibyl-gateway-cache::Cache` for the source of truth.
    ...
    /// request (AISIX-Cloud#410). Surfaces in the `x-sibylhub-served-by`
    ...
    /// Surfaces in the `x-sibylhub-route` response header so callers can see
...
struct SemanticGateCtx {
    ...
    embed_entry: Arc<sibyl_gateway_core::ResourceEntry<sibyl_gateway_core::Model>>,
    policy_id: String,
    policy_name: String,
    generation: u32,
...
/// the backend is failing is `sibyl_gateway_redis_failures_total{operation}`; the
...
async fn resolve_cache_hit(
    ...
    match cache.get(key).await {
        ...
        // `sibyl_gateway_redis_failures_total`, and a cache Redis can now stay
...
pub(crate) fn estimation_output_text(resp: &sibyl_gateway_hub::ChatResponse) -> String {
...
fn estimate_missing_chat_usage(
    resp: &sibyl_gateway_hub::ChatResponse,
    upstream_model: &str,
    req: &ChatFormat,
) -> (u32, u32, bool) {
...
fn apply_estimated_usage(
    usage: &mut sibyl_gateway_hub::chat::UsageStats,
    prompt_tokens: u32,
...
fn estimate_subcall_tokens(
    req: &ChatFormat,
    model: &str,
    usage: &sibyl_gateway_hub::chat::UsageStats,
    output_text: &str,
) -> (u32, u32, bool) {
...
pub(crate) struct EffectiveSubcallUsage {
    pub(crate) usage: sibyl_gateway_hub::chat::UsageStats,
...
pub(crate) fn effective_subcall_usage(
    req: &ChatFormat,
    model: &str,
    reported: &sibyl_gateway_hub::chat::UsageStats,
    output_text: &str,
...
fn last_user_message_text(req: &ChatFormat) -> Option<String> {
    req.messages
        .iter()
        .rev()
        .find(|m| m.role == sibyl_gateway_hub::Role::User)
...
/// `x-sibylhub-served-by` response header is emitted only when the
...
mod served_by_target_tests {
    ...
    fn direct_model_returns_none_even_when_a_target_was_captured() {
        ...
        // never carry `x-sibylhub-served-by`.
        assert_eq!(
...
async fn dispatch(
    ...
    *audit_out = resolved.audit_log();
    let resolved_chain: std::sync::Arc<dyn sibyl_gateway_guardrails::Guardrail> =
    ...
    // non-semantic request) for the `x-sibylhub-route` response header.
    ...
    if req.is_streaming() {
        ...
        // (AISIX-Cloud#1087).
        let mut won_member_reservation: Option<sibyl_gateway_ratelimit::MultiReservation> = None;
        ...
        'targets: for (target_idx, attempt) in attempt_models.iter().enumerate() {
            ...
            for attempt_idx in 0..=budget.attempts {
                ...
                let attempt_stream: Result<sibyl_gateway_hub::ChatChunkStream, BridgeError> =
                            ...
                            if timeouts.stream_configured {
                                ...
                                match up.next().await {
                                    ...
                                    Some(Ok(chunk)) => Ok(Box::pin(
                                        ...
                                        as sibyl_gateway_hub::ChatChunkStream),
                                    Some(Err(e)) => Err(e),
        ...
        let sse_stream = build_sse_stream(
            ...
            sibyl_gateway_hub::chat::UsageStats::default(),
            Some(estimator),
        ...
        return Ok(Success {
            ...
            // crates/sibyl-gateway-cache/src/lib.rs. Always surface as
    ...
    // `snapshot.cache_policies` (see sibyl-gateway-etcd). Stage 4 will add
    ...
    let semantic_gate: Option<SemanticGateCtx> =
        match (matched_policy.as_ref(), cache_key_full.as_ref()) {
            (Some(entry), Some(key_full)) => entry.value.semantic.as_ref().and_then(|cfg| {
                ...
                let embedding_model = sibyl_gateway_core::models::resolve_model_ref(
                    snapshot,
                    &cfg.embedding_model,
    ...
    // to populate the `x-sibylhub-served-by` response header for routing
    // requests (AISIX-Cloud#410). Stays `None` until an attempt wins.
    let mut chosen_target_display_name: Option<String> = None;
    let mut upstream: Option<sibyl_gateway_hub::ChatResponse> = None;
    let retry_on_429 = virtual_entry
        .value
        .routing
        .as_ref()
...
async fn dispatch_ensemble(
    ...
    auth: &AuthenticatedKey,
    snapshot: &sibyl_gateway_core::GatewaySnapshot,
    virtual_entry: &sibyl_gateway_core::ResourceEntry<sibyl_gateway_core::Model>,
    req: &ChatFormat,
    request_id: &str,
    created_ts: i64,
    reservation: sibyl_gateway_ratelimit::MultiReservation,
    resolved_chain: &Arc<dyn sibyl_gateway_guardrails::Guardrail>,
    ...
    if req.is_streaming() {
        ...
        let panel_usage_sum = panel
            .iter()
            .fold(sibyl_gateway_hub::chat::UsageStats::default(), |acc, p| {
        ...
        return Ok(Success {
            ...
            telemetry_handled_by_stream: true,
            // Not a routing request — no `x-sibylhub-served-by` header.
            served_by_target: None,
            served_by_route: None,
            routing: RoutingTelemetry::default(),
            captured_content: None,
    ...
    let emit_subcalls = |outcome: &crate::ensemble::EnsembleOutcome,
                         ...
                         hits: &[sibyl_gateway_core::GuardrailMonitorHit],
                         terminal_judge: bool| {
...
fn record_budget_gauges(
    ...
    let labels = sibyl_gateway_obs::BudgetLabels {
        api_key_id: &auth.entry.id,
    ...
    if let Some(budget) = budget {
        metrics.set_budget_gauges(
            labels,
            sibyl_gateway_obs::BudgetGauges {
...
fn emit_usage_event(
    ...
    let mut event = UsageEvent {
        ...
        // Source struct is `sibyl_gateway_core::TelemetryTags`; the wire
    ...
    // `sibyl_gateway_usage_events_emitted_total` (#408). Keep `&'static str`
...
fn emit_access_log(
    ...
    cache: Option<sibyl_gateway_obs::CacheAccessLog<'_>>,
) {
...
struct StreamGuardrailContext {
    chain: Arc<dyn sibyl_gateway_guardrails::Guardrail>,
...
fn build_sse_stream<F>(
    ...
    base_usage: sibyl_gateway_hub::chat::UsageStats,
    ...
    F: FnOnce(StreamCompletion) + Send + 'static,
{
    ...
    let inner = async_stream::stream! {
        ...
        let mut pending: Vec<sibyl_gateway_hub::ChatChunk> = Vec::new();
        ...
        // terminates abnormally, sibyl-gateway sends a final error chunk
        ...
        while let Some(item) = upstream.next().await {
            let maybe_chunk = match item {
                Ok(mut chunk) => {
                    ...
                    if let Some(text) = chunk.delta.content.as_deref() {
                        if matches!(
                            stream_policy,
                            sibyl_gateway_guardrails::StreamOutputPolicy::Window { .. }
                        ) {
                    ...
                    {
                        for tc in tcs {
                            if buf.len() >= sibyl_gateway_guardrails::DEFAULT_STREAM_OUTPUT_BUFFER_BYTES {
                                break;
                            }
                            if let Some(f) = tc.get("function") {
                                if let Some(n) = f.get("name").and_then(|v| v.as_str()) {
            ...
            if !hold_back || cap_released {
                ...
                match &stream_policy {
                    sibyl_gateway_guardrails::StreamOutputPolicy::Window {
                        ...
                        if window_buf.chars().count() >= *size_chars {
                            if let Some(ctx) = output_guardrail.as_ref() {
                                let synthesized = {
                                    let comp = guard.comp();
                                    sibyl_gateway_hub::ChatResponse {
                                        ...
                                        usage: sibyl_gateway_hub::UsageStats::new(
                                            comp.prompt_tokens,
                                            comp.completion_tokens,
                                        ),
                                    }
                                ...
                                match verdict {
                                    ...
                                    sibyl_gateway_guardrails::GuardrailVerdict::Bypass { reason } => {
                                        let comp = guard.comp();
                    ...
                    sibyl_gateway_guardrails::StreamOutputPolicy::EndOfStreamCheck => {}
                }
            }
        ...
        if !errored && !cap_released {
            if hold_back {
                ...
                if let Some(ctx) = output_guardrail.as_ref() {
                    let content_part = match &stream_policy {
                        sibyl_gateway_guardrails::StreamOutputPolicy::Window { .. } => window_buf.clone(),
                    ...
                    let blocked = if final_text.is_empty() {
                        ...
                        let synthesized = {
                            ...
                            sibyl_gateway_hub::ChatResponse {
                                id: comp.provider_request_id.clone(),
                                model: comp.provider_model_version.clone(),
                                message: sibyl_gateway_hub::ChatMessage::assistant(final_text),
                                finish_reason: sibyl_gateway_hub::FinishReason::Stop,
                                usage: sibyl_gateway_hub::UsageStats::new(
                                    comp.prompt_tokens,
                                    comp.completion_tokens,
                ...
                (content_buffer.as_ref(), output_guardrail.as_ref())
            {
                ...
                let synthesized = sibyl_gateway_hub::ChatResponse {
                    id: guard.comp().provider_request_id.clone(),
                    model: guard.comp().provider_model_version.clone(),
                    message: sibyl_gateway_hub::ChatMessage::assistant(scan_text),
                    finish_reason: sibyl_gateway_hub::FinishReason::Stop,
                    usage: sibyl_gateway_hub::UsageStats::new(
                ...
                match verdict {
                    ...
                    sibyl_gateway_guardrails::GuardrailVerdict::Allow => {}
                }
            }
        }
    ...
    // `x-sibylhub-request-id` (AISIX-Cloud#1060).
...
mod cooldown_tests {
    use super::*;
    use sibyl_gateway_core::CooldownConfig;
...
mod complete_on_drop_tests {
    ...
    use sibyl_gateway_hub::chat::UsageStats;
    ...
    fn drop_and_capture_with_estimator(
        ...
        let cap = captured.clone();
        let delivered = Arc::new(AtomicU32::new(delivered_count));
        let req = sibyl_gateway_hub::chat::ChatFormat::new(
            "relay-model",
            vec![sibyl_gateway_hub::chat::ChatMessage {
                role: sibyl_gateway_hub::chat::Role::User,
    ...
    fn chat_response_for_estimation(
        text: &str,
        usage: UsageStats,
    ) -> sibyl_gateway_hub::chat::ChatResponse {
        sibyl_gateway_hub::chat::ChatResponse {
            id: "id".into(),
            model: "relay-model".into(),
            message: sibyl_gateway_hub::chat::ChatMessage {
                role: sibyl_gateway_hub::chat::Role::Assistant,
            ...
            finish_reason: sibyl_gateway_hub::chat::FinishReason::Stop,
            usage,
        }
    }

    fn subcall_req(user: &str) -> sibyl_gateway_hub::chat::ChatFormat {
        sibyl_gateway_hub::chat::ChatFormat::new(
            "relay-model",
            vec![sibyl_gateway_hub::chat::ChatMessage {
                role: sibyl_gateway_hub::chat::Role::User,
    ...
    fn estimate_subcall_fills_missing_usage() {
        let usage = sibyl_gateway_hub::chat::UsageStats::default();
        let (prompt, completion, estimated) =
    ...
    fn estimate_subcall_preserves_reported_usage() {
        let usage = sibyl_gateway_hub::chat::UsageStats {
            prompt_tokens: 17,
            completion_tokens: 23,
            total_tokens: 40,
            ..Default::default()
    ...
    fn effective_subcall_usage_is_cache_inclusive_and_preserves_overhead() {
        let reported = sibyl_gateway_hub::chat::UsageStats {
            prompt_tokens: 7,
