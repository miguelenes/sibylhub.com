...
impl Usage {
    ...
    fn from_stats(u: &sibyl_gateway_hub::UsageStats) -> Self {
...
pub fn inject_ratelimit_headers(
    ...
    status: &sibyl_gateway_ratelimit::RateLimitStatus,
) {
...
mod tests {
    use super::*;
    use sibyl_gateway_hub::{ChatMessage, UsageStats};
    ...
    fn render_chunk_emits_usage_details_on_terminal_chunk() {
        ...
        let chunk = ChatChunk {
            id: "c".into(),
            model: "m".into(),
            delta: sibyl_gateway_hub::ChatDelta {
                role: None,
                content: None,
                tool_calls: None,
