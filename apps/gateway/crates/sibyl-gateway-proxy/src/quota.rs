...
use sibyl_gateway_ratelimit::MultiReservation;
...
fn match_conditional_layer(
    ...
    if !sibyl_gateway_core::models::eval_condition_nodes(
...
async fn reserve_policy_layers(
    ...
    input: &ConditionInput<'_>,
    phase: PolicyPhase,
    reservations: &mut Vec<sibyl_gateway_ratelimit::Reservation>,
) -> Result<(), ProxyError> {
...
/// counting it under `sibyl_gateway_ratelimit_rejections_total{scope,layer}` —
...
fn reject(
    state: &ProxyState,
    err: sibyl_gateway_ratelimit::RateLimitError,
    layer: &'static str,
...
async fn check_budget(state: &ProxyState, auth: &AuthenticatedKey) -> Result<(), ProxyError> {
    ...
    let budget_labels = sibyl_gateway_obs::BudgetLabels {
...
mod tests {
    ...
    fn make_auth(team_id: Option<&str>, user_id: Option<&str>) -> AuthenticatedKey {
        ...
        AuthenticatedKey {
            anonymous: false,
            entry: std::sync::Arc::new(sibyl_gateway_core::resource::ResourceEntry::new(
                "key-entry-1",
                key,
                1,
            )),
