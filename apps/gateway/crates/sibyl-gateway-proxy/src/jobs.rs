...
//! `model` field, `?model=` query, or `x-sibylhub-model` header); the file id
...
//!
//! Ids minted by the gateway look like `sibyl-gateway-<base64url>`; raw provider
...
const ROUTED_ID_PREFIX: &str = "sibyl-gateway-";
...
/// sibyl-gateway-provider-azure-openai (kept as a local const — this crate
...
/// `sibyl-gateway-<base64url_nopad("<raw>;model,<display_name>")>`.
...
pub(crate) struct JobTarget {
    ...
    /// gateway's own.
    forwarded_client: sibyl_gateway_hub::ForwardedClientHeaders,
}
...
/// Explicit routing hint from `?model=` / `x-sibylhub-model` header.
...
pub(crate) fn resolve_target(
    ...
    let forwarded_client = sibyl_gateway_hub::ForwardedClientHeaders::resolve(&header_ctx);
    let extra_headers = sibyl_gateway_hub::resolve_default_headers(&header_ctx);
    Ok(JobTarget {
        model_entry,
        pk_entry,
        secret,
...
async fn send_upstream(
    ...
    // e.g. `x-sibylhub-request-id` on the wire.
...
async fn scan_input_blob(
    ...
    enforced_hits: &mut Vec<sibyl_gateway_core::GuardrailEnforcedHit>,
    scores: &mut Vec<sibyl_gateway_core::GuardrailScore>,
...
fn emit_job_usage_event(
    ...
    // (AISIX-Cloud#1330).
    guardrail_enforced_hits: Vec<sibyl_gateway_core::GuardrailEnforcedHit>,
...
fn finish(
    ...
    enforced_hits: Vec<sibyl_gateway_core::GuardrailEnforcedHit>,
    ...
    match result {
        Ok((mut resp, target)) => {
            ...
            if let Ok(hv) = HeaderValue::from_str(&request_id) {
                resp.headers_mut().insert("x-sibylhub-request-id", hv);
            }
            resp
        }
        Err(err) => {
...
pub(crate) async fn get_file(
    ...
    auth: AuthenticatedKey,
    client: ClientContext,
    GatewayPath(id): GatewayPath<String>,
    Query(params): Query<HashMap<String, String>>,
    headers: HeaderMap,
) -> Response {
...
pub(crate) async fn create_batch(
    ...
    let mut enforced_hits: Vec<sibyl_gateway_core::GuardrailEnforcedHit> = Vec::new();
    let mut scores: Vec<sibyl_gateway_core::GuardrailScore> = Vec::new();
...
fn batch_attribution_request_id(raw_batch_id: &str, idx: usize, multi: bool) -> String {
    let name = if multi {
        format!("sibyl-gateway:batch:{raw_batch_id}:{idx}")
    } else {
        format!("sibyl-gateway:batch:{raw_batch_id}")
    };
...
async fn attribute_batch_usage(
    ...
    model_id: &str,
    display_name: &str,
    cost: Option<&sibyl_gateway_core::models::model::ModelCost>,
    pk_id: &str,
    secret: &str,
    adapter: Adapter,
    api_base: Option<&str>,
    conn: Option<&sibyl_gateway_core::models::provider_key::UpstreamConnection>,
    raw_batch_id: &str,
    output_file_id: &str,
) -> Result<(), String> {
...
mod tests {
    ...
    fn openai_pk(id: &str, api_base: &str) -> ResourceEntry<sibyl_gateway_core::ProviderKey> {
        let json = format!(
    ...
    fn azure_pk(id: &str, api_base: &str) -> ResourceEntry<sibyl_gateway_core::ProviderKey> {
        let json = format!(
    ...
    fn anthropic_pk(id: &str, api_base: &str) -> ResourceEntry<sibyl_gateway_core::ProviderKey> {
        let json = format!(
    ...
    fn routed_id_roundtrip() {
        ...
        assert_eq!(decode_routed_id("sibyl-gateway-!!!!"), None);
    }
    ...
    fn seed_blocking_input_guardrail(snap: &GatewaySnapshot) {
        ...
        crate::seed_env_scoped_guardrail(
            snap,
            sibyl_gateway_core::resource::ResourceEntry::new("g-in", g, 1),
        );
    }
    ...
    fn seed_blocking_output_guardrail(snap: &GatewaySnapshot) {
        let g: sibyl_gateway_core::Guardrail = serde_json::from_str(
        ...
        crate::seed_env_scoped_guardrail(
            snap,
            sibyl_gateway_core::resource::ResourceEntry::new("g-out", g, 1),
        );
    }
