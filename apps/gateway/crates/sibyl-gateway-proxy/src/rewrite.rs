...
use sibyl_gateway_core::UrlRewriteRule;
...
pub async fn rewrite_request_uri(
    ...
    let fired = state.url_rewrites.iter().find_map(|rule| {
        if let Some(hosts) = &rule.hosts {
            if !host
                .as_deref()
                .is_some_and(|host| sibyl_gateway_core::host::matches(hosts, host))
            {
                return None;
            }
        }
...
mod tests {
    ...
    fn router_with_rules(rules: Vec<UrlRewriteRule>) -> axum::Router {
        ...
        let cfg = sibyl_gateway_core::ProxyConfig {
        ...
        let state = ProxyState::new(
            SnapshotHandle::new(sibyl_gateway_core::GatewaySnapshot::new()),
