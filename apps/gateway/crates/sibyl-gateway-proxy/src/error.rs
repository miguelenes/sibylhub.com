...
pub enum ProxyError {
    ...
    /// expiry. See [`sibyl_gateway_core::WhenAllUnavailablePolicy`].
...
impl ProxyError {
    ...
    pub fn envelope(&self) -> ErrorEnvelope {
        ...
        if let ProxyError::Bridge(sibyl_gateway_hub::BridgeError::UpstreamStatus {
            status,
            message,
            parsed,
            wire,
        ...
        if let ProxyError::Bridge(sibyl_gateway_hub::BridgeError::UpstreamInBand {
            status,
            message,
            parsed,
            wire,
...
fn render_bridge_upstream_envelope(
    status: u16,
    message: &str,
    parsed: Option<&sibyl_gateway_hub::UpstreamErrorView>,
    wire: sibyl_gateway_hub::UpstreamWire,
) -> ErrorEnvelope {
    let is_4xx = (400..500).contains(&status);
    if is_4xx && !matches!(wire, sibyl_gateway_hub::UpstreamWire::Unknown) {
...
impl ProxyError {
    ...
    /// `x-ratelimit-*` trio on a 429 the caller's signal that SibylHub Gateway
    /// itself rejected the request rather than relaying somebody else's
    /// rejection.
    fn rate_limit_detail(&self) -> Option<sibyl_gateway_ratelimit::LimitDetail> {
        match self {
...
/// gateway. `X-RateLimit-Scope` is an SibylHub Gateway extension naming which
...
fn apply_retry_headers(
    ...
    detail: Option<sibyl_gateway_ratelimit::LimitDetail>,
) {
...
mod tests {
    ...
    fn timeout_cause_reaches_logs_but_not_the_caller() {
        let err = ProxyError::Bridge(sibyl_gateway_hub::BridgeError::Timeout {
            elapsed_ms: 5_002,
    ...
    async fn gateway_429_carries_the_standard_rate_limit_headers() {
        let err = ProxyError::RateLimit(sibyl_gateway_ratelimit::RateLimitError::Requests {
            scope: sibyl_gateway_core::RateLimitScope::Requests,
            detail: sibyl_gateway_ratelimit::LimitDetail {
                dimension: "rpm",
                limit: 100,
                remaining: 0,
                reset_secs: 43,
    ...
    async fn anthropic_envelope_429_carries_the_same_rate_limit_headers() {
        let err = ProxyError::RateLimit(sibyl_gateway_ratelimit::RateLimitError::Tokens {
    ...
    async fn policy_429_reports_the_policy_layer_limit() {
        let err = ProxyError::PolicyRateLimit {
            source: sibyl_gateway_ratelimit::RateLimitError::Requests {
                scope: sibyl_gateway_core::RateLimitScope::Requests,
                detail: sibyl_gateway_ratelimit::LimitDetail {
                    dimension: "rpd",
                    limit: 5_000,
    ...
    async fn concurrency_429_carries_all_four_headers() {
        let err = ProxyError::RateLimit(sibyl_gateway_ratelimit::RateLimitError::Concurrency {
            detail: sibyl_gateway_ratelimit::LimitDetail {
                dimension: sibyl_gateway_ratelimit::CONCURRENCY_DIMENSION,
                limit: 8,
                remaining: 0,
                reset_secs: sibyl_gateway_ratelimit::CONCURRENCY_RETRY_AFTER_SECS,
            },
        });
        let resp = err.into_response();
        let h = resp.headers();
