...
//! `sibyl_gateway_proxy_requests_total` sample. "Client reports 413, gateway has no
...
pub(crate) fn reject_before_dispatch(
    ...
    .emit();
    // `path` must be normalized, not passed through: `GatewayPath` below hands
...
pub(crate) struct GatewayPath<T>(pub(crate) T);

#[axum::async_trait]
impl<T> FromRequestParts<ProxyState> for GatewayPath<T>
where
...
mod tests {
    ...
    async fn wiring_bugs_keep_their_500_instead_of_blaming_the_caller() {
        ...
        async fn miswired(
            crate::reject::GatewayPath(_x): crate::reject::GatewayPath<String>,
