...
//! the jobs surface (`jobs::encode_routed_id`), minus the `sibyl-gateway-` prefix
...
fn resolve_video_target(
    ...
    let base_url = match crate::dispatch::resolve_base_url(&pk_entry.value) {
        ...
        Err(_) if video_provider == VideoProvider::Openai => {
            sibyl_gateway_provider_openai::OPENAI_DEFAULT_BASE.to_string()
        }
        Err(e) => return Err(e),
    };
...
async fn provider_call(
    ...
    let note = |e: sibyl_gateway_hub::BridgeError| {
    ...
    let retry_permit = |e: &sibyl_gateway_hub::BridgeError| {
        method == reqwest::Method::GET
            || !matches!(e, sibyl_gateway_hub::BridgeError::UpstreamDecode(_))
    };
    crate::routing::retrying_dispatch_gated(
        ...
        || {
            ...
            // two values of e.g. `x-sibylhub-request-id` on the wire.
            ...
            if let Ok(v) = header::HeaderValue::from_str(request_id) {
                headers.insert(header::HeaderName::from_static("x-sibylhub-request-id"), v);
            }
            ...
            async move {
                ...
                if !(200..300).contains(&status) {
                    ...
                    return Err(note(sibyl_gateway_hub::BridgeError::upstream_status(
                        status, message,
                    )));
                }

                serde_json::from_slice(&bytes).map_err(|e| {
                    sibyl_gateway_hub::BridgeError::UpstreamDecode(format!(
...
async fn proxy_content(
    ...
    if !status.is_success() {
        ...
        return Err(ProxyError::Bridge(note(
            sibyl_gateway_hub::BridgeError::upstream_status(code, message),
        )));
    }
...
struct Telemetry<'a> {
    ...
    /// with `sibyl_gateway_proxy_in_flight_requests`. Coarser than `path`, which
    ...
    *audit_out = resolved_chain.audit_log();
    ...
    if !resolved_chain.is_empty() {
        let chat = sibyl_gateway_hub::ChatFormat::new(
            &body.model,
            vec![sibyl_gateway_hub::ChatMessage::user(body.prompt.clone())],
        );
        let (verdict, hits) =
...
pub async fn get_video(
    ...
    auth: AuthenticatedKey,
    client: ClientContext,
    GatewayPath(video_id): GatewayPath<String>,
) -> Response {
    let telemetry = Telemetry {
        state: &state,
        method: "GET",
...
mod tests {
    ...
    fn new_snap(api_base: &str, provider: &str, model_extra: &str) -> GatewaySnapshot {
    ...
    fn openai_default_base_composes_videos_url() {
        ...
        assert_eq!(
            VideoProvider::Openai.submit_url(sibyl_gateway_provider_openai::OPENAI_DEFAULT_BASE),
            "https://api.openai.com/v1/videos"
        );
    }
