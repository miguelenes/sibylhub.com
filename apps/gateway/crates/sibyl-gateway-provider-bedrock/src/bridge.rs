...
fn build_client(
    ...
    let aws_creds = Credentials::new(
        ...
        None,
        "sibyl-gateway-provider-bedrock",
    );
...
fn filtered_extra_headers(hdr: &UpstreamHeaderContext<'_>) -> Vec<(String, String)> {
    ...
    let forwarded = sibyl_gateway_hub::ForwardedClientHeaders::resolve(hdr);
    sibyl_gateway_hub::resolve_extra_headers(hdr)
        .into_iter()
        .filter(|(name, _)| {
...
fn map_service_error(
    ...
    let retry_after = sibyl_gateway_hub::parse_retry_after(&hdrs);
    ...
    BridgeError::UpstreamStatus {
        ...
        message,
        parsed,
        wire: sibyl_gateway_hub::UpstreamWire::Bedrock,
        retry_after,
    }
}
...
impl Bridge for BedrockBridge {
    ...
    fn wire_protocol(&self) -> &'static str {
        sibyl_gateway_core::Adapter::Bedrock.wire_protocol()
    }
...
mod tests {
        ...
        for (svc, want_status) in cases {
            ...
            match map_converse_stream_event_error(e) {
                BridgeError::UpstreamInBand {
                    ...
                    assert!(matches!(wire, sibyl_gateway_hub::UpstreamWire::Bedrock));
                    assert!(
    ...
    async fn chat_maps_upstream_429_with_retry_after_and_canned_rate_limited() {
        ...
        match err {
            BridgeError::UpstreamStatus {
                ...
                assert_eq!(wire, sibyl_gateway_hub::UpstreamWire::Bedrock);
    ...
    async fn chat_anthropic_translates_system_messages_to_system_field() {
        ...
        // `split_system` helper from sibyl-gateway-provider-anthropic must
    ...
    async fn chat_converse_maps_upstream_429_with_retry_after_and_bedrock_wire() {
        ...
        match err {
            BridgeError::UpstreamStatus {
                ...
                assert_eq!(
                    wire,
                    sibyl_gateway_hub::UpstreamWire::Bedrock,
    ...
    async fn a_messages_request_to_bedrock_claude_keeps_its_optional_properties() {
        ...
        .unwrap()
        .clone();
        sibyl_gateway_provider_anthropic::wire::translate_extras_to_openai_shape(
            &mut extra,
            sibyl_gateway_core::MappedEffort::AsWritten,
        );
        let mut req = ChatFormat::new("my-model", vec![ChatMessage::user("who are you")]);
        req.extra = extra;
...
mod forwarded_header_tests {
    use super::*;
    use sibyl_gateway_core::RequestOverrides;
