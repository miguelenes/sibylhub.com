...
use sibyl_gateway_provider_openai::wire::{
...
use sibyl_gateway_hub::{apply_request_headers, UpstreamHeaderContext};
...
impl VertexBridge {
    ...
    /// (`crates/sibyl-gateway-provider-bedrock/src/bridge.rs::build_client_from_ctx`).
...
async fn map_http_error(status: StatusCode, resp: reqwest::Response) -> BridgeError {
    ...
    let parsed = kind.as_ref().map(|_| {
        Box::new(sibyl_gateway_hub::UpstreamErrorView {
            kind: kind.clone(),
            message: None,
            code: None,
            param: None,
    ...
    BridgeError::UpstreamStatus {
        ...
        message,
        parsed,
        wire: sibyl_gateway_hub::UpstreamWire::Vertex,
        retry_after,
    }
}
...
fn parse_vertex_openai_stream_payload(
    ...
    serde_json::from_str(data).map_err(|e| {
        sibyl_gateway_hub::capture_in_band_error(data, sibyl_gateway_hub::UpstreamWire::Vertex)
...
impl Bridge for VertexBridge {
    ...
    fn wire_protocol(&self) -> &'static str {
        sibyl_gateway_core::Adapter::Vertex.wire_protocol()
    }
    ...
    async fn embed(
        ...
        let url = sibyl_gateway_hub::url_cache::cached_model_endpoint_url(
...
impl VertexBridge {
    ...
    async fn chat_anthropic_stream(
        ...
        // StreamState. Mirrors sibyl-gateway-provider-anthropic's
        ...
        let stream = async_stream::try_stream! {
            ...
            while let Some(item) = byte_stream.next().await {
                let bytes: Bytes = item.map_err(|e| BridgeError::Transport(sibyl_gateway_hub::transport_error_message(&e)))?;
                for event in decoder.feed(bytes.as_ref()) {
                    ...
                    if let AnthropicStreamEvent::Error { error } = &parsed {
                        Err(sibyl_gateway_provider_anthropic::wire::stream_error_into_bridge_error(error))?;
                    }
                    state.update(&parsed);
    ...
    async fn chat_gemini_stream(
        ...
        // `sibyl-gateway-provider-openai::build_chunk_stream`.
        let stream = async_stream::try_stream! {
            let mut decoder = SseDecoder::new();
...
/// `Content-Type: application/json`, `x-sibylhub-request-id`. The Bearer
    ...
    async fn chat_gemini_maps_4xx_to_canned_message_not_body_echo() {
        ...
        match err {
            BridgeError::UpstreamStatus {
                ...
                assert_eq!(wire, sibyl_gateway_hub::UpstreamWire::Vertex);
        ...
        while let Some(item) = stream.next().await {
            match item {
                ...
                Err(BridgeError::UpstreamInBand {
                    ...
                    assert!(matches!(wire, sibyl_gateway_hub::UpstreamWire::Vertex));
                    saw_in_band = true;
                    break;
                }
...
data: {\"type\":\"error\",\"error\":{\"type\":\"overloaded_error\",\"message\":\"Overloaded\"}}\n\n"
        ...
        while let Some(item) = stream.next().await {
            match item {
                ...
                Err(BridgeError::UpstreamInBand { status, wire, .. }) => {
                    ...
                    assert!(
                        matches!(wire, sibyl_gateway_hub::UpstreamWire::Anthropic),
    ...
    // exist (those are unit-tested in `sibyl-gateway-provider-openai::overrides`).
