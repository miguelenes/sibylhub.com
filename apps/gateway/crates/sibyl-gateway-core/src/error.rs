...
//!   (spec §10).
//!
//! Framework-specific `IntoResponse` impls live in the `sibyl-gateway-proxy` / `sibyl-gateway-admin`
...
/// Bootstrap-time failures (config load, TLS, bind). Returned by `sibyl-gateway-server`.
#[derive(Debug, Error)]
