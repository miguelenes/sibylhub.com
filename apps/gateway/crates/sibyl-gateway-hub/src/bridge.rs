...
//! Model's [`sibyl_gateway_core::Provider`] enum.
//!
...
/// from the [`sibyl_gateway_core::GatewaySnapshot`]. Bridges read from it but
/// do not mutate it.
#[derive(Clone)]
...
impl BridgeError {
    ...
    /// error was raised.
    ///
    /// Gates the `sibyl_gateway_deployment_*` families, which read as **upstream
...
/// individual `sibyl-gateway-provider-*` crates.
#[async_trait]
