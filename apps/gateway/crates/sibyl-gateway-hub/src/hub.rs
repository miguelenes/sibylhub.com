...
/// in sibyl-gateway-server pins the agreement against the real registrations.
///
...
mod tests {
    ...
    async fn registered_bridge_is_callable() {
        ...
        let m = std::sync::Arc::new(
            serde_json::from_str::<sibyl_gateway_core::Model>(
                r#"{"display_name":"t","provider":"openai","model_name":"gpt-4o","provider_key_id":"pk-1"}"#,
            )
            .unwrap(),
        );
        let pk = std::sync::Arc::new(
            serde_json::from_str::<sibyl_gateway_core::ProviderKey>(
    ...
    fn pk(provider: &str, adapter: Option<&str>) -> sibyl_gateway_core::ProviderKey {
        ...
        serde_json::from_str::<sibyl_gateway_core::ProviderKey>(&json).unwrap()
    }
