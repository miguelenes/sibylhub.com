...
//! [`StreamOutputPolicy`] in `sibyl-gateway-proxy`'s `build_sse_stream`; each
...
struct AigDiagnostics {
    ...
    /// `request_id`, which is the gateway's own id (`x-sibylhub-request-id`)
...
mod tests {
    ...
    //     cargo test -p sibyl-gateway-guardrails aliyun_ai_guardrail::tests::live_smoke \
