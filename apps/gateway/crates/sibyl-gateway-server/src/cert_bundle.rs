...
//!
//!   SIBYL_GATEWAY_MANAGED__CP_CERT_PEM   — leaf certificate
//!   SIBYL_GATEWAY_MANAGED__CP_KEY_PEM    — SEC1 EC private key paired with
//!                                  the leaf
//!   SIBYL_GATEWAY_MANAGED__CP_CA_PEM     — CA cert the DP installs as the
...
//! pick-one.
//!
//! The cert's URI SAN encodes `x-sibyl-gateway://env/<env_id>` per cp-api's
//! `internal/dpmgr/certmgr/issue.go`. We parse it out and plant it
//! into `cfg.etcd.env_id` so `effective_prefix()` returns
//! `/sibyl-gateway/<env_id>/` — the same scoping the legacy register-derived
...
pub struct Provisioned {
    ...
    /// (`x-sibyl-gateway://env/<env_id>`). Plumbed into `cfg.etcd.env_id` so
    /// every Range/Watch lands on `/sibyl-gateway/<env_id>/`.
    pub env_id: String,
    /// dp_id parsed from the leaf cert's URI SAN
    /// (`x-sibyl-gateway://dp/<dp_id>`). Used for heartbeat payload + log
    /// correlation.
    pub dp_id: String,
...
pub async fn provision(cfg: &ManagedConfig) -> anyhow::Result<Provisioned> {
    ...
    if env_id.is_empty() {
        bail!("leaf cert is missing the `x-sibyl-gateway://env/<uuid>` SAN URI");
    }
    if dp_id.is_empty() {
        bail!("leaf cert is missing the `x-sibyl-gateway://dp/<uuid>` SAN URI");
    }
...
fn parse_san_uris(pem: &str) -> anyhow::Result<(String, String)> {
    ...
    for name in &san.value.general_names {
        if let GeneralName::URI(uri) = name {
            if let Some(rest) = uri.strip_prefix("x-sibyl-gateway://env/") {
                env_id = rest.to_string();
            } else if let Some(rest) = uri.strip_prefix("x-sibyl-gateway://dp/") {
