//! Secret redaction for `sibyl-gateway export`.
//!
...
//! It deliberately does **not** start with `SIBYL_GATEWAY_`: the gateway's
//! config loader owns that prefix (`Environment::with_prefix("SibylHub Gateway")`),
...
//! conventions keep secret variables off the `SIBYL_GATEWAY_` prefix.

use serde_json::Value;

/// Namespace every derived secret variable shares. Config-safe (does not
/// begin with the `SIBYL_GATEWAY_` config-override prefix) and greppable.
pub const SECRET_ENV_PREFIX: &str = "SIBYLSECRET";
...
/// `SIBYLSECRET_<KIND>_<IDENTITY>_<FIELD>` — the derived variable name.
...
mod tests {
    ...
    fn redact_top_level_replaces_value_and_records_placeholder() {
        ...
        assert_eq!(
            out[0].env_var,
            "SIBYLSECRET_PROVIDER_KEY_OPENAI_PROD_API_KEY"
        );
    ...
    fn redact_by_key_reaches_nested_bedrock_secret() {
        ...
        assert_eq!(
            ...
            json!("${SIBYLSECRET_GUARDRAIL_BEDROCK_GUARD_SECRET_ACCESS_KEY}")
        );
        assert_eq!(out.len(), 2);
    }
    ...
    fn redact_string_map_masks_string_values_and_keeps_non_strings() {
        ...
        assert_eq!(
            ...
            json!("${SIBYLSECRET_PROVIDER_KEY_PK_DEFAULT_HEADERS_X_TENANT_TOKEN}")
        );
    ...
    fn redact_headers_masks_each_value() {
        ...
        assert_eq!(
            ...
            json!("${SIBYLSECRET_OBSERVABILITY_EXPORTER_HONEYCOMB_HEADER_X_HONEYCOMB_TEAM}")
        );
        assert_eq!(out.len(), 1);
