//! `sibyl-gateway export` — emit a resources file from a running etcd store.
//!
//! The inverse of the file source (`sibyl_gateway_core::filesource`): it reads
...
//!
//! ```text
//! etcd range read → decode (sibyl_gateway_etcd::build_snapshot, the loader path)
    ...
    if !document.blocking.is_empty() {
        eprintln!(
            "\n{} issue(s) leave the exported file non-loadable — fix them in the source and \
             re-export (or run `sibyl-gateway validate` on the file):",
...
fn report_rejections(stats: &sibyl_gateway_etcd::BuildStats) {
    if stats.rejections.is_empty() {
        return;
    }
    eprintln!(
