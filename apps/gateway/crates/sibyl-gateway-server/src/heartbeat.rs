...
//! (`x-sibyl-gateway://env/<env_id>/dp/<dp_id>`) to derive the *credential*
...
/// User-Agent). The base version is [`sibyl_gateway_core::BUILD_VERSION`] —
/// release builds are stamped from the git tag, local builds fall back
/// to the crate version. CI additionally stamps `SIBYL_GATEWAY_BUILD_SHA` — the
...
/// `sibyl_gateway_core::Config` normalises it at load.
...
/// separate from `sibyl_gateway_obs::SinkStatsSnapshot` so the obs crate's
...
/// Kept separate from `sibyl_gateway_etcd::loader::PartialCompatEntry` so the
...
async fn send(client: &reqwest::Client, cfg: &HeartbeatConfig, uptime: i64) -> anyhow::Result<()> {
    ...
    let resp = client
        ...
        .json(&HeartbeatBody {
            ...
            supported_guardrail_kinds: sibyl_gateway_guardrails::supported_kinds(),
            applied_revision,
            config_hash,
            exporter_health,
...
///
/// Public so the BudgetClient (sibyl-gateway-proxy) and any future per-request
/// CP caller can reuse the same identity without duplicating the PEM-
/// loading dance. `sibyl-gateway-server::telemetry` keeps its own copy because
...
mod tests {
    ...
    fn build_version_appends_stamped_image_sha() {
        ...
        // An empty stamp (e.g. `SIBYL_GATEWAY_BUILD_SHA=` in a misconfigured CI
    ...
    fn write_test_bundle(dir: &Path) -> MtlsBundle {
        // Self-signed CA: subject = issuer = "sibyl-gateway-test-ca".
...
"#
    ...
    async fn send_includes_guardrail_kinds_revision_and_exporter_health() {
        ...
        let cfg = cfg_with_bundle(format!("{}/dp/heartbeat", server.uri()), mtls)
            ...
            .with_exporter_health_fetcher(Arc::new(|| {
                ...
                m.insert(
                    ...
                    sibyl_gateway_obs::SinkStatsSnapshot {
                        delivered_batches: 3,
                        failed_batches: 1,
                ...
                m.insert(
                    ...
                    sibyl_gateway_obs::SinkStatsSnapshot::default(),
                );
                m
            }));
        ...
        // the schema vocabulary in `sibyl_gateway_guardrails`.
        assert_eq!(
            body["supported_guardrail_kinds"],
            serde_json::json!(sibyl_gateway_guardrails::supported_kinds()),
        );
            ...
            .with_rejection_fetcher(Arc::new(|| {
                vec![
                    ConfigRejectionSnapshot {
                        key: "/sibyl-gateway/models/stale-served".into(),
                    ...
                    ConfigRejectionSnapshot {
                        key: "/sibyl-gateway/models/never-loaded".into(),
                        kind: "schema_failed".into(),
                        error: "schema validation failed".into(),
                        timestamp_unix_secs: 1_770_000_100,
    ...
    async fn send_includes_partially_compatible_resources_when_wired() {
        ...
        let cfg = cfg_with_bundle(format!("{}/dp/heartbeat", server.uri()), mtls)
            .with_partial_compat_fetcher(Arc::new(|| {
                vec![sibyl_gateway_etcd::loader::PartialCompatEntry {
                    kind: "api_keys".into(),
