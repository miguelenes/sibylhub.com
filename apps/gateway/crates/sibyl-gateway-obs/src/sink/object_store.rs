...
fn upstream_client_options() -> object_store::ClientOptions {
    client_options_for(sibyl_gateway_hub::upstream_http::config())
}
...
fn client_options_for(
    cfg: &sibyl_gateway_hub::upstream_http::UpstreamHttpConfig,
...
/// not `SIBYL_GATEWAY_`, which the config loader owns). Returns `None` when a required
...
mod tests {
    ...
    fn the_configured_upstream_connection_settings_reach_the_exporter_client_options() {
        ...
        let cfg = sibyl_gateway_hub::upstream_http::UpstreamHttpConfig {
    ...
    fn the_production_accessor_reads_the_process_upstream_config() {
        ...
        let from_process_config = client_options_for(sibyl_gateway_hub::upstream_http::config());
        assert_ne!(
    ...
    /// of these. The sibling scan in `sibyl-gateway-hub`'s `upstream_http`
    ...
    async fn build_sink_with_missing_credentials_is_broken() {
        ...
        let cfg = ObjectStoreConfig {
            ...
            credential_ref: "sibyl_gateway_objstore_edge_unset_ref".into(),
        };
...
/// `cargo test -p sibyl-gateway-obs -- --ignored objstore_smoke`.
#[cfg(test)]
mod smoke {
    use super::{
    ...
    async fn smoke_roundtrip(store: Arc<dyn ObjectStore>) {
        let prefix = format!("sibyl-gateway-smoke/{}", uuid::Uuid::new_v4());
    ...
    #[ignore = "hits a real S3 or S3-compatible server (native AWS / MinIO / LocalStack). \
                Run: docker compose -f crates/sibyl-gateway-obs/tests/object-store-emulators.compose.yml up -d \
                then SIBYL_GATEWAY_E2E_OBJSTORE_S3_* set; cargo test -p sibyl-gateway-obs -- --ignored objstore_smoke_s3"]
    async fn objstore_smoke_s3_roundtrip() {
        let (Some(bucket), Some(key_id), Some(secret)) = (
            env("SIBYL_GATEWAY_E2E_OBJSTORE_S3_BUCKET"),
            env("SIBYL_GATEWAY_E2E_OBJSTORE_S3_ACCESS_KEY_ID"),
            env("SIBYL_GATEWAY_E2E_OBJSTORE_S3_SECRET_ACCESS_KEY"),
        ) else {
            eprintln!("objstore_smoke_s3: SIBYL_GATEWAY_E2E_OBJSTORE_S3_* not set — skipping");
            return;
        };
    ...
    #[ignore = "hits a real Azure Blob account (or the Azurite emulator). \
                Run with SIBYL_GATEWAY_E2E_OBJSTORE_AZURE_* set; \
                cargo test -p sibyl-gateway-obs -- --ignored objstore_smoke_azure"]
    async fn objstore_smoke_azure_roundtrip() {
        let (Some(container), Some(account), Some(access_key)) = (
            env("SIBYL_GATEWAY_E2E_OBJSTORE_AZURE_CONTAINER"),
        ...
        let endpoint = env("SIBYL_GATEWAY_E2E_OBJSTORE_AZURE_ENDPOINT");
        let store = build_object_store(
            ObjectStoreProvider::AzureBlob,
            &container,
            None,
    ...
    #[tokio::test]
    #[ignore = "GCS round-trip — set SIBYL_GATEWAY_E2E_OBJSTORE_GCS_* against REAL GCS or a \
                ...
                cargo test -p sibyl-gateway-obs -- --ignored objstore_smoke_gcs"]
    async fn objstore_smoke_gcs_roundtrip() {
        let (Some(bucket), Some(service_account_key)) = (
            env("SIBYL_GATEWAY_E2E_OBJSTORE_GCS_BUCKET"),
            env("SIBYL_GATEWAY_E2E_OBJSTORE_GCS_SERVICE_ACCOUNT"),
        ) else {
            eprintln!("objstore_smoke_gcs: SIBYL_GATEWAY_E2E_OBJSTORE_GCS_* not set — skipping");
            return;
        };
    ...
    #[ignore = "KEYLESS cloud_identity S3 round-trip — set \
                SIBYL_GATEWAY_E2E_OBJSTORE_CLOUDID_S3_BUCKET to a real bucket the runtime's \
                AMBIENT identity can write (EC2 instance role / EKS IRSA / \
                GitHub-OIDC-assumed role). No static keys: from_env sources the \
                ambient chain. cargo test -p sibyl-gateway-obs -- --ignored \
                objstore_smoke_s3_cloud_identity"]
    async fn objstore_smoke_s3_cloud_identity() {
        let Some(bucket) = env("SIBYL_GATEWAY_E2E_OBJSTORE_CLOUDID_S3_BUCKET") else {
            eprintln!(
                "objstore_smoke_s3_cloud_identity: SIBYL_GATEWAY_E2E_OBJSTORE_CLOUDID_S3_BUCKET not set — skipping"
            );
            return;
        };
    ...
    #[ignore = "KEYLESS cloud_identity GCS round-trip — set \
                SIBYL_GATEWAY_E2E_OBJSTORE_CLOUDID_GCS_BUCKET on a real GKE pod with Workload \
                ...
                key. cargo test -p sibyl-gateway-obs -- --ignored objstore_smoke_gcs_cloud_identity"]
    async fn objstore_smoke_gcs_cloud_identity() {
        let Some(bucket) = env("SIBYL_GATEWAY_E2E_OBJSTORE_CLOUDID_GCS_BUCKET") else {
            eprintln!(
                "objstore_smoke_gcs_cloud_identity: SIBYL_GATEWAY_E2E_OBJSTORE_CLOUDID_GCS_BUCKET not set — skipping"
            );
            return;
        };
