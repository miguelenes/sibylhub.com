...
/// deliberately NOT `SIBYL_GATEWAY_`: that namespace is owned by the config loader
/// (`Environment::with_prefix("SibylHub Gateway")`), which reads such a name as a
...
mod tests {
    ...
    fn base_url_prepends_project_for_a_region_host() {
        assert_eq!(
            base_url_for("ap-southeast-3.log.aliyuncs.com", "sibyl-gateway-obs"),
            "https://sibyl-gateway-obs.ap-southeast-3.log.aliyuncs.com"
        );
    ...
    fn base_url_uses_a_scheme_qualified_endpoint_verbatim() {
        ...
        assert_eq!(
            base_url_for("http://mock-sls:9000", "sibyl-gateway-obs"),
            "http://mock-sls:9000"
        );
    }
