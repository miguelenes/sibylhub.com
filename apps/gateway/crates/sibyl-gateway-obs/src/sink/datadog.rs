...
impl DatadogSink {
    ...
    /// under an `sibyl-gateway.` prefix. The Datadog reserved attributes (`ddsource`,
    ...
    fn to_log(&self, record: &SinkRecord) -> Value {
        ...
        // (or `sibyl-gateway.`) key. Empty strings are skipped; numeric 0 / false stay.
...
/// that have a convention; everything else keeps an `sibyl-gateway.`-prefixed custom
...
fn map_field_name(field: &str) -> String {
    match field {
        ...
        // ── SibylHub Gateway custom dimensions (no semconv) ──
        other => return format!("sibyl-gateway.{other}"),
    }
    .to_string()
}
...
/// `DD_CRED_<REF>_API_KEY`. The prefix is deliberately NOT `SIBYL_GATEWAY_`: that
/// namespace is owned by the config loader (`Environment::with_prefix("SibylHub Gateway")`),
...
mod tests {
    ...
    async fn appends_batch_posts_gzipped_json_with_api_key_header() {
        ...
        let log = &logs[0];
        assert_eq!(log["ddsource"], "sibyl-gateway-ai-gateway");
        ...
        // SibylHub Gateway custom dimensions under the `sibyl-gateway.` prefix.
        assert_eq!(log["sibyl-gateway.request_id"], "req-42");
        assert_eq!(log["sibyl-gateway.model_id"], "gpt-4o");
        assert_eq!(log["sibyl-gateway.upstream_latency_ms"], 123);
        // The request's kind, under the `sibyl-gateway.` prefix every field without a
        ...
        assert_eq!(log["sibyl-gateway.operation"], "image_generation");
        ...
        // Empty metadata is omitted uniformly (no blank `sibyl-gateway.api_key_id`).
        assert!(log.get("sibyl-gateway.api_key_id").is_none());
