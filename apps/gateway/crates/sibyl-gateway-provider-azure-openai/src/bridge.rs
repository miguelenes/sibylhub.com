...
//! `sibyl-gateway-provider-openai::wire`). Azure differs on three axes:
//!
...
impl AzureOpenAiBridge {
    ...
    fn resolve_url(
    ...
    ) -> sibyl_gateway_hub::url_cache::EndpointUrl {
        #[cfg(test)]
        if let Some(u) = &self.url_override {
            ...
            return sibyl_gateway_hub::url_cache::EndpointUrl::Unparsed(u.clone());
        }
        sibyl_gateway_hub::url_cache::cached_model_endpoint_url(
            &ctx.provider_key_id,
            "azure/chat",
            &upstream.deployment,
...
async fn map_http_error(status: StatusCode, resp: reqwest::Response) -> BridgeError {
    ...
    BridgeError::UpstreamStatus {
        ...
        message,
        parsed,
        wire: sibyl_gateway_hub::UpstreamWire::AzureOpenAI,
        retry_after,
    }
}
...
/// `x-sibylhub-request-id: <ctx.request_id>`, and (for streaming)
...
impl Bridge for AzureOpenAiBridge {
    ...
    fn wire_protocol(&self) -> &'static str {
        sibyl_gateway_core::Adapter::AzureOpenai.wire_protocol()
    }
...
mod tests {
    ...
    fn a_translated_messages_request_reaches_azure_byte_for_byte_as_before() {
        ...
        use sibyl_gateway_provider_anthropic::wire::translate_extras_to_openai_shape;
        ...
        .unwrap()
        .clone();
        translate_extras_to_openai_shape(&mut extra, sibyl_gateway_core::MappedEffort::AsWritten);
    ...
    fn parse_stream_chunk_captures_in_band_error_frame() {
        ...
        match err {
            BridgeError::UpstreamInBand {
                ...
                assert!(matches!(wire, sibyl_gateway_hub::UpstreamWire::AzureOpenAI));
            }
    ...
    fn build_request_headers_uses_api_key_not_bearer() {
        ...
        assert_eq!(headers.get("x-sibylhub-request-id").unwrap(), "req-1");
        assert!(
    ...
    async fn chat_400_with_inner_error_responsible_ai_lifts_kind() {
        ...
        match err {
            BridgeError::UpstreamStatus {
                ...
                assert_eq!(status, 400);
                assert_eq!(wire, sibyl_gateway_hub::UpstreamWire::AzureOpenAI);
    ...
    async fn chat_stream_yields_chunks_until_done_marker_with_inline_content_filters() {
        ...
        assert_eq!(
            ...
            Some(sibyl_gateway_hub::Role::Assistant)
        );
