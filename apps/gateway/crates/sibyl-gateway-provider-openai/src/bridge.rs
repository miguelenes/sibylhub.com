...
//!   [`sibyl_gateway_core::Model`]'s `provider_config`.
        ...
        let raw = match ctx.provider_key.api_base.as_deref() {
            ...
            _ => {
                ...
                if !pk_vendor_normalized.is_empty() && pk_vendor_normalized != "openai" {
                    ...
                    tracing::error!(
                        target: "sibyl_gateway_provider_openai::bridge",
...
fn parse_openai_error_envelope(body: &[u8]) -> Option<sibyl_gateway_hub::UpstreamErrorView> {
...
/// x-sibylhub-request-id, and optionally Accept: text/event-stream
...
impl Bridge for OpenAiBridge {
    ...
    fn wire_protocol(&self) -> &'static str {
        sibyl_gateway_core::Adapter::Openai.wire_protocol()
    }
...
mod tests {
    ...
    /// Audit fix (PR #323): the [`sibyl_gateway_hub::MAX_UPSTREAM_ERROR_BODY_BYTES`]
    ...
    async fn non_streaming_oversize_error_body_truncated_to_max_message_bytes() {
        ...
        match err {
            BridgeError::UpstreamStatus { message, .. } => {
                ...
                // alignment back-off.
                assert!(
                    message.len() <= sibyl_gateway_hub::MAX_UPSTREAM_ERROR_MESSAGE_BYTES + 8,
    ...
    async fn non_streaming_oversize_parsed_message_truncated() {
        ...
        match err {
            BridgeError::UpstreamStatus {
                ...
                assert!(
                    pm.len() <= sibyl_gateway_hub::MAX_UPSTREAM_ERROR_MESSAGE_BYTES + 8,
...
data: {\"error\":{\"message\":\"The server had an error processing your request\",\"type\":\"server_error\",\"code\":\"server_error\"}}\n\n";
        ...
        match err {
            BridgeError::UpstreamInBand {
                ...
                assert!(matches!(wire, sibyl_gateway_hub::UpstreamWire::OpenAI));
            }
            other => panic!("unexpected: {other:?}"),
        }
...
data: [DONE]\n";
    ...
    async fn chat_with_no_overrides_behaves_like_pre_d2() {
        ...
        Mock::given(method("POST"))
            .and(path("/chat/completions"))
            .and(header("authorization", "Bearer sk-test"))
            .and(header("x-sibylhub-request-id", "req-1"))
            .respond_with(ResponseTemplate::new(200).set_body_json(ok_chat_response()))
            .mount(&server)
            .await;
    ...
    // The previous "issue #368: X-SibylHub Gateway-Bridge outbound header" block
