...
//! on [`sibyl_gateway_core::ProviderKey`]); Phase D consumes the functions
...
//! live in `sibyl-gateway-core` so cp-api can write them straight into etcd
...
/// etcd, so it stays in the provider crate rather than `sibyl-gateway-core`.
